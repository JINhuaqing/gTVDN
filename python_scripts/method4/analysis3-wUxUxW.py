...
def corr_plot(vecs, cutoff=0.05, trun_fn=None):
    ...
    sns.heatmap(abs_corrMat,  yticklabels=kysOrd, cmap="coolwarm", center=0, vmin=-1, vmax=1, annot=np.round(abs_corrMat, 2))
...
nets7_path = list(paras.data_dir.glob("DK_dic68.csv"))[0]
...
for key, vec in dict_nets7.items():
    ...
    np.savetxt(paras.res_dir/f"./cann_{key}.txt", out_vec)
...
# save centers 
for ix in range(n_clus):
    cur_vec = clus.cluster_centers_[ix, :]
    np.savetxt(paras.res_dir/f"./clu_centers_{n_clus}_{ix+1}.txt", cur_vec)
...
# wegithed U, U x w
cens_wU = np.matmul(np.array(B1), clu_wU.T)
plt_wUs = cens_wU

tfn1 = partial(cumsum_trunc, cutoff=0.9)
corr_plot(plt_wUs, trun_fn=tfn1);
#plt.savefig(paras.fig_dir/f"corr_wxU.jpg", bbox_inches='tight')

#save_brainplotting_vec(plt_wUs, filname_prefix=f"clu_wxU_{n_clus}", is_csv=True)

wxU_FC_top_eigmodes = []
for ix in range(n_clus):
    curU = np.array(plt_wUs[:, ix])
    FC = obt_FC_exp(curU, normalize=0.03)
    res = eig_sorted(FC)
    top_eigmode = res[1][:, 0]
    print(np.abs(top_eigmode).min(), np.median(np.abs(top_eigmode)), np.max(np.abs(top_eigmode)))
    wxU_FC_top_eigmodes.append(np.abs(top_eigmode))
    
#save_brainplotting_vec(wxU_FC_top_eigmodes, filname_prefix=f"wxU_FC_top_{n_clus}")
...
# wegithed U, absU x w
cens_wU = np.matmul(np.abs(B1), clu_wU.T)
plt_wUs = cens_wU

tfn1 = partial(cumsum_trunc, cutoff=0.9)
corr_plot(plt_wUs, cutoff=0.05, trun_fn=tfn1);

#plt.savefig(paras.fig_dir/f"corr_wxabsU.jpg", bbox_inches='tight')
#save_brainplotting_vec(plt_wUs, filname_prefix=f"clu_wxabsU_{n_clus}", is_csv=True)

wxabsU_FC_top_eigmodes = []
for ix in range(n_clus):
    curU = np.array(plt_wUs[:, ix])
    FC = obt_FC_exp(curU, normalize=0.03)
    res = eig_sorted(FC)
    top_eigmode = res[1][:, 0]
    print(np.abs(top_eigmode).min(), np.median(np.abs(top_eigmode)), np.max(np.abs(top_eigmode)))
    wxabsU_FC_top_eigmodes.append(np.abs(top_eigmode))
    
#save_brainplotting_vec(wxabsU_FC_top_eigmodes, filname_prefix=f"wxabsU_FC_top_{n_clus}")


# In[226]:


# wegithed U, abs(U x w)
cens_wU = np.matmul(np.array(B1), clu_wU.T)
plt_wUs = np.abs(cens_wU)

tfn1 = partial(cumsum_trunc, cutoff=0.9)
corr_plot(plt_wUs, cutoff=0.05, trun_fn=tfn1);
#plt.savefig(paras.fig_dir/f"corr_abswxU.jpg", bbox_inches='tight')
#save_brainplotting_vec(plt_wUs, filname_prefix=f"clu_abswxU_{n_clus}", is_csv=True)

abswxU_FC_top_eigmodes = []
for ix in range(n_clus):
    curU = np.array(plt_wUs[:, ix])
    FC = obt_FC_exp(curU, normalize=0.03)
    res = eig_sorted(FC)
    top_eigmode = res[1][:, 0]
    print(np.abs(top_eigmode).min(), np.median(np.abs(top_eigmode)), np.max(np.abs(top_eigmode)))
    abswxU_FC_top_eigmodes.append(np.abs(top_eigmode))
    
#save_brainplotting_vec(abswxU_FC_top_eigmodes, filname_prefix=f"abswxU_FC_top_{n_clus}")
