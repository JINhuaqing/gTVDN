...
# This file analyze the results for double regressions.
# 
# 1. First reg to select fs
# 
# 2. Second Reg to get AUC
...
gopt_C1 = LOO_bestC(reg_X_std, reg_Y, paras.Cs, 1)
print(gopt_C1)
...
final_clf = LogisticRegression(random_state=0, C=gopt_C1, penalty=paras.penalty, solver="liblinear").fit(reg_X_std, reg_Y)
...
for _ in trange(rep_num):
    ...
    cur_clf = LogisticRegression(penalty=paras.penalty, random_state=0, C=gopt_C1, solver="liblinear")
...
# select fs
reg_X_std_selected = reg_X_std[:, keep_idx_id_boot]
reg_X_std_selected.shape


# In[29]:


gopt_C2 = LOO_bestC(reg_X_std_selected, reg_Y, paras.Cs, 1)
print(gopt_C2)
...
pred_probs = LOO_pred_givenC(reg_X_std_selected, reg_Y, Cs=gopt_C2, is_prg=1)
...
def run_fn(ix):
    cur_X_test = reg_X_std_selected[ix, :].reshape(1, -1)
...
probs = []
pred_probs = LOO_pred_givenC(reg_X_std_selected, reg_Y, Cs=best_Cs, is_prg=1)
fpr, tpr, thresholds = roc_curve(reg_Y, pred_probs, pos_label=1)
auc = roc_auc_score(reg_Y, pred_probs)
...
np.random.seed(0)
nobs = reg_X_std_selected.shape[0]
rep_aucs = []
for j in tqdm(range(10000)):
    test_idxs = np.random.choice(nobs, int(nobs/5), False)
    train_idxs = np.delete(np.arange(nobs), test_idxs)
    clf = LogisticRegression(penalty=paras.penalty, random_state=0, C=gopt_C2, solver="liblinear")
    clf.fit(reg_X_std_selected[train_idxs], reg_Y[train_idxs])
    cur_eprobs = clf_2probs(clf, reg_X_std_selected[test_idxs, :])
